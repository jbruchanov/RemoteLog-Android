...
public final class ServerConstants {
    ...
    /**
     * The sender_id contained in the registration_id does not match the sender_id used to register
     * with the GCM servers.
     */
    public static final String ERROR_MISMATCH_SENDER_ID = "MismatchSenderId";

    /**
     * The user has uninstalled the application or turned off notifications. Sender should stop
     * sending messages to this device and delete the registration_id. The client needs to
     * re-register with the GCM servers to receive notifications again.
     */
    ...
    /**
     * A particular message could not be sent because the GCM servers were not available. Used only
     * on JSON requests, as in plain text requests unavailability is indicated by a 503 response.
     */
    public static final String ERROR_UNAVAILABLE = "Unavailable";

    /**
     * A particular message could not be sent because the GCM servers encountered an error. Used
     * only on JSON requests, as in plain text requests internal errors are indicated by a 500
     * response.
     */
